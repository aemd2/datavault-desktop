...
export function useNotionPages(connectorId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("notion_pages")
        .select("id, connector_id, title, parent_id, last_edited_time, url, notion_properties:raw_json->properties")
      ...
      if (error) {
        let qBasic = supabase
          .from("notion_pages")
          .select("id, connector_id, title, parent_id, last_edited_time, url")
          .order("last_edited_time", { ascending: false });
